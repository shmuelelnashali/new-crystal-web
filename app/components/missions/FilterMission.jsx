...
export default function FilterMission({
  ...
  const clearOption = (labelName) => {
    setFormData((prev) => ({
      ...prev,
      [labelName]: labelName === "department" ? [] : "", // מערך ריק עבור department
    }));
  };
                    ...
                    labelName === "department" && (
                      <div className="w-full relative mt-1">
                        <MadorsToggle
                          ...
                          selectedMadors={formData.department || []}
                          valueToEdit={formData[labelName]}
                          fieldName={"department"}
                        />
                        {/* המדורים שנבחרו */}
                        {formData.department?.length > 0 && (
                          <div className="flex bg-[#002A781F] rounded-md flex-wrap gap-2 mt-1 p-2">
                            {formData.department.map((mador, index) => (
                              <div
                                key={index}
                                className="bg-[#002A78] text-white   rounded-md text-sm flex items-center gap-1"
                              >
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    handleInputChange("department", newMadors);
                                  }}
                                  className="ml-1 hover:text-red-900 hover:cursor-pointer hover:bg-red-300"
                                >
                                  ×
                                </button>
                              </div>
...
// שדות של דרישות אמצעים
const department = ["מדור 12", "מדור 65", "מדור 99", "מדור 453"];
const officer_name = ["אברהם", "משה", "שרה", "סרן יעקב רז"];
const task_status = ["חדשה", "מבוטלת", "נפתחה משימה"];
